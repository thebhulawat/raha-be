...
export default async function createOrUpdateSchedule(req: Request, res: Response) {
  try {
    ...
    const existingSchedule = await db.select().from(scheduleTable).where(eq(scheduleTable.userId, user[0].id));

    let resultSchedule: SelectSchedule;

    if (existingSchedule.length > 0) {
      // Update existing schedule
      const [updatedSchedule] = await db.update(scheduleTable)
        .set(scheduleData)
      ...
      const [insertedSchedule] = await db.insert(scheduleTable).values(scheduleData).returning();
      resultSchedule = insertedSchedule;
    }

    res.status(200).json({
      message: existingSchedule.length > 0 ? 'Schedule updated successfully' : 'Schedule created successfully',
      schedule: resultSchedule
    });

  } catch (error) {
    console.error('Error saving/updating schedule:', error);
    res.status(500).json({ error: 'An error occurred while saving/updating the schedule' });
  }
}